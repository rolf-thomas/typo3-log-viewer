...
| Datei | Plattform |
|-------|-----------|
| `typo3-log-viewer-0.12.0-macos-arm64.tar.gz` | macOS Apple Silicon |
| `typo3-log-viewer-0.12.0-macos-x86_64.tar.gz` | macOS Intel |
| `typo3-log-viewer-0.12.0-linux-x86_64-musl.tar.gz` | Linux x86_64 (statisch, max. Portabilität) |
| `typo3-log-viewer-0.12.0-linux-arm64.tar.gz` | Linux ARM64 |

Das Binary `typo3-log-viewer-0.12.0-linux-x86_64-musl.tar.gz` kann bei den meisten shared Hosting Anbietern in `~/.local/bin` plaziert von überall aufgerufen werden.

```bash
curl -L https://github.com/rolf-thomas/typo3-log-viewer/releases/download/v0.12.0/typo3-log-viewer-0.12.0-linux-x86_64-musl.tar.gz | tar -xz
```
